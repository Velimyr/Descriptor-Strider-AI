...
async function startServer() {
  ...
  app.get("/api/auth/google/url", (req, res) => {
    ...
    const url = client.generateAuthUrl({
      ...
      prompt: "consent",
      state: redirectUri // Pass redirectUri in state to retrieve it in callback
    });
    res.json({ url });
  });
  ...
  app.post("/api/sheets/append", async (req, res) => {
    ...
    try {
      ...
      // Get spreadsheet to find sheetId by name
      ...
      // Prepare values - handle images
      const rows = Array.isArray(values[0]) ? values : [values];
      const processedRows = rows.map(row => 
        row.map(cell => {
          if (typeof cell === 'string' && cell.startsWith('data:image/')) {
            // Google Sheets IMAGE function doesn't support base64 directly
            // We'll just put the text for now or a placeholder
            return "[Зображення]";
          }
          return cell;
        })
  ...
  app.post("/api/sheets/delete-rows", async (req, res) => {
    ...
    try {
      ...
      // 2. Identify rows to delete (URL is usually at index tableStructure.length, Page at index + 1)
      // We'll search for rows where any cell matches the URL AND any cell matches the page number
      // More precisely, we know we append them at the end.

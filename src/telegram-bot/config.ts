...
export interface TelegramBotConfig {
  ...
  slicing: {
    ...
    jpegQuality: number;
    // Розширення кожної знайденої зони у нормалізованих одиницях (0..1)
    // після розпізнавання Gemini. Корисне бо модель часто обрізає верх/низ справи.
    bboxPaddingX: number; // горизонтальний padding (рідко потрібен)
    bboxPaddingY: number; // вертикальний padding (зазвичай 0.01–0.02 = 1–2% висоти)
    alignBoxesHorizontally: boolean; // вирівняти всі зони сторінки до спільної ширини
  };
...
export const telegramBotConfig: TelegramBotConfig = {
  ...
  slicing: {
    ...
    autoPrompt:
      'Ти бачиш скан сторінки архівного опису справ. Це таблиця з кількома колонками: порядковий номер | назва справи | роки | кількість аркушів | примітки. Кожен рядок (або кілька суміжних рядків) — окрема архівна справа.\n\n' +
      'Знайди прямокутні зони — по одній на кожну справу — і поверни JSON-масив у форматі Gemini detect:\n' +
      '[{"box_2d": [ymin, xmin, ymax, xmax], "label": "case 1"}, ...]\n' +
      'де координати — цілі числа 0..1000, нормалізовані до розміру зображення, ymin<ymax, xmin<xmax.\n\n' +
      'КРИТИЧНО ВАЖЛИВО ПРО ШИРИНУ:\n' +
      '• Кожна зона має охоплювати ВСЮ ШИРИНУ ТАБЛИЦІ — від лівої межі першої колонки (порядковий номер) до правої межі останньої колонки (примітки).\n' +
      '• xmin для ВСІХ зон на сторінці має бути ОДНАКОВИМ — приблизно на лівому краю таблиці (зазвичай xmin ≈ 30..80).\n' +
      '• xmax для ВСІХ зон на сторінці має бути ОДНАКОВИМ — приблизно на правому краю таблиці (зазвичай xmax ≈ 920..980).\n' +
      '• НЕ виділяй лише одну колонку (лише назву чи лише дату) — потрібен ВЕСЬ рядок таблиці зліва направо.\n\n' +
      'КРИТИЧНО ВАЖЛИВО ПРО ВИСОТУ:\n' +
      '• Кожна зона охоплює всі рядки однієї справи — від першого рядка з порядковим номером до останнього рядка з продовженням назви/приміткою.\n' +
      '• ymin — НАД першим рядком справи (із запасом ~5 пікселів). ymax — ПІД останнім рядком (із запасом ~5 пікселів). Краще охопити трохи зайвого ніж обрізати справу навпіл.\n' +
      '• Не зливай дві сусідні справи в одну зону.\n' +
      '• Ігноруй заголовок таблиці (рядок з назвами колонок) і колонтитули сторінки.\n\n' +
      'Поверни ТІЛЬКИ JSON-масив, без markdown і коментарів.',
    renderScale: 2.0,

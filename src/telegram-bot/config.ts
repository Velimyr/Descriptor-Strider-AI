...
export const telegramBotConfig: TelegramBotConfig = {
  ...
  slicing: {
    ...
    autoPrompt:
      'Ти бачиш скан сторінки архівного опису справ. Це таблиця, де кожен рядок (або кілька суміжних рядків) — окрема архівна справа з реквізитами: порядковий номер, заголовок, роки, кількість аркушів тощо.\n\n' +
      ...
      'КРИТИЧНО ВАЖЛИВО:\n' +
      '• Кожна зона має повністю охоплювати ВСІ рядки таблиці, що належать одній справі — від ПЕРШОГО рядка з порядковим номером до ОСТАННЬОГО рядка з продовженням назви/приміткою.\n' +
      '• ymin завжди має бути НАД першим рядком справи (із запасом ~5 пікселів зверху), ymax — ПІД останнім рядком (із запасом ~5 пікселів знизу). Краще охопити трохи зайвого ніж обрізати справу навпіл.\n' +
      '• xmin/xmax мають охопити всі колонки таблиці зліва направо, а не лише одну колонку.\n' +
      '• Не зливай дві сусідні справи в одну зону. Зони можуть трохи перекриватись по вертикалі — це нормально.\n' +

...
| `TELEGRAM_ADMIN_LOGIN` | логін суперадміна (напр. `admin`) | вигадайте |
| `TELEGRAM_ADMIN_PASSWORD` | пароль суперадміна | вигадайте сильний |
| `ADMIN_SESSION_SECRET` | підпис сесійних токенів адмінки | `openssl rand -hex 32` |
...
> 🔒 **Адмінка прихована від звичайних користувачів.** Кнопки в UI немає — заходите за прямим URL `https://your-domain.vercel.app/#telegram-admin`. Далі — логін/пароль.
>
> Акаунтів два типи: **суперадмін з env** (`TELEGRAM_ADMIN_LOGIN` / `TELEGRAM_ADMIN_PASSWORD`) — аварійний вхід, який працює завжди, і **модератори** з таблиці `bot_admins`, яких суперадмін створює у вкладці «Модератори» (див. розділ 5.1).
...
## 5.1. Модератори з обмеженим доступом

Щоб дати комусь частину адмінки — не весь набір вкладок:

1. Увійдіть суперадміном (env-логін) → вкладка **«Модератори»** (видима лише суперадмінам).
2. **«Додати модератора»** → логін (латиниця, 3–32 символи), імʼя для себе, і галочки навпроти вкладок, які йому потрібні.
3. Сервер згенерує пароль і покаже його **один раз** — перепишіть і передайте людині. Відновити його неможливо, лише скинути (кнопка **«Скинути пароль»**).
4. Модератор заходить на той самий `#telegram-admin` і бачить тільки дозволені вкладки. Спроба смикнути чужий ендпоінт напряму дає `403`.

Корисно знати:

- **Зміна прав, скидання пароля чи вимкнення акаунта миттєво розлогінює** модератора — його поточний токен перестає діяти на наступному ж запиті.
- Галочка **«Суперадмін»** дає всі права разом із керуванням модераторами. Зняти її з себе або вимкнути власний акаунт не можна — щоб не залишитись без доступу.
- Скоуп **«Шоу»** керує доступом до сторінки `/quiz` (вікторина + архівний стендап).
- Хто саме забанив користувача чи запустив розсилку, видно в полях `banned_by` / `created_by` — там тепер логін модератора.
...
- `TELEGRAM_CRON_SECRET` — захищає `/cron/*` (GitHub Actions). В адмінку він більше **не** потрапляє.
- `ADMIN_SESSION_SECRET` — ним підписуються сесійні токени адмінки. Токен несе id акаунта і його `token_epoch`; права звіряються з БД на кожному запиті, тож відкликання діє миттєво. Зміна цього секрету розлогінює всіх адмінів одразу.
- Кожен `/api/telegram/admin/*` проходить через один guard зі списком дозволених шляхів: ендпоінт, не вписаний у цей список, повертає `403` — випадково відкритих роутів не буває.
- Адмін-UI зберігає токен у `localStorage` — не діліться браузером.

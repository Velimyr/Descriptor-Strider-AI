...
-- Бан користувача ("Перевірка доброчесності") — див. коментар у schema.sql.
alter table botdev_users add column if not exists banned     boolean     not null default false;
alter table botdev_users add column if not exists ban_reason text;
...
-- Бали СПІЛЬНІ з вікториною — та сама таблиця botdev_quiz_scores.

create table if not exists botdev_standup_state (
  id              int         primary key default 1 check (id = 1),
  session_id      text        not null default '',
  status          text        not null default 'idle' check (status in ('idle', 'running', 'finished')),
  q_index         int         not null default 0,
...
insert into botdev_standup_state (id) values (1) on conflict (id) do nothing;
...
create table if not exists botdev_standup_signups (
  session_id   text        not null,
  q_index      int         not null,
  tg_id        text        not null,
  display_name text        not null default '',
...
create index if not exists idx_dev_standup_signups_round on botdev_standup_signups(session_id, q_index, created_at);

-- Лайки. PK = один лайк від глядача конкретному виступаючому в конкретному раунді.
create table if not exists botdev_standup_votes (
  session_id      text        not null,
  q_index         int         not null,
...
create index if not exists idx_dev_standup_votes_round on botdev_standup_votes(session_id, q_index);
...
create table if not exists botdev_standup_rounds (
  session_id text        not null,
  q_index    int         not null,
...
alter table botdev_standup_state   enable row level security;
alter table botdev_standup_signups enable row level security;
alter table botdev_standup_votes   enable row level security;
alter table botdev_standup_rounds  enable row level security;

-- Нарахування за раунд: усі, хто набрав максимум лайків, отримують p_points у
-- спільний рейтинг (botdev_quiz_scores). Ідемпотентно: повторний виклик повертає
...
drop function if exists botdev_standup_award_round(text, int, int);
create or replace function botdev_standup_award_round(p_session text, p_q_index int, p_points int)
...
begin
  ...
  if v_winners is null then
    ...
    if v_max is null or v_max = 0 then
      ...
      left join botdev_standup_signups s
      ...
      on conflict (tg_id) do update
        set points       = botdev_quiz_scores.points + p_points,
            wins         = botdev_quiz_scores.wins + 1,
            display_name = coalesce(nullif(excluded.display_name, ''), botdev_quiz_scores.display_name),
            updated_at   = now();
    end if;

    insert into botdev_standup_rounds(session_id, q_index, winners)
